...
import dev.application.common.book.text.ReadText;
...
import dev.application.common.dto.ReadTextInputDTO;
import dev.application.common.dto.ReadTextOutputDTO;
...
import dev.application.common.logic.RDataOutputSubLogic;
...
public class ReadSoccerCsvDataUpd {
	...
	private void setCsvMapData() {
		...
		for (String path : bookList) {
			...
			ThresHoldEntity entities = readFileOutputDTO.getReadHoldDataList().get(0);

			List<String> mapList = new ArrayList<String>();
			mapList.add(entities.getDataCategory());
	...
	private void remakeCsvFileAndRegisterFileChkTmp(String csv_number, String dataCategoryKey,
		...
		if (file2.exists()) {
			...
			if (selectResultList != null && !selectResultList.isEmpty()) {
				...
				List<FileChkTmpEntity> insertList = new ArrayList<FileChkTmpEntity>();
				FileChkTmpEntity fileChkTmpEntity = new FileChkTmpEntity();
