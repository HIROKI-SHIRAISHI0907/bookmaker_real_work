package dev.application.common.file;
...
public class ReadThresHoldFile {
	...
	public ReadFileOutputDTO getFileBody(String fileFullPath) {
		...
		List<ThresHoldEntity> entiryList = new ArrayList<ThresHoldEntity>();
		try (BufferedReader br = new BufferedReader(
			...
			while ((text = br.readLine()) != null) {
				...
				if (row > 0) {
					...
					ThresHoldEntity mappingDto = new ThresHoldEntity();
					mappingDto.setSeq(parts[0]);
					mappingDto.setConditionResultDataSeqId(parts[1]);
					mappingDto.setDataCategory(parts[2]);
					mappingDto.setTimes(parts[3]);
					...
					mappingDto.setHomeBlockShoot(parts[20]);
					mappingDto.setAwayBlockShoot(parts[21]);
					mappingDto.setHomeBigChance(parts[22]);
					mappingDto.setAwayBigChance(parts[23]);
					mappingDto.setHomeCorner(parts[24]);
					mappingDto.setAwayCorner(parts[25]);
