 ...
 *
 */
public class AnalyzeThresholdGettingScoreEachFeatureLogic {
	...
	public void execute() throws InterruptedException {
		...
		EachFeatureSubLogic eachFeatureSubLogic = new EachFeatureSubLogic();
		...
		for (String file : findBookOutputDTO.getBookList()) {
			...
			try {
				List<String> fileSubList = eachFeatureSubLogic.execute(entityList);
				for (String files : fileSubList) {
					if (!fileList.contains(files)) {
						fileList.add(files);
					}
				}
			} catch (Exception e) {
