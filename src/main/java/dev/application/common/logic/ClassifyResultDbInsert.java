...
public class ClassifyResultDbInsert {
	 ...
	 * @param file_name
	 */
	public void execute(List<ClassifyResultDataEntity> insertEntities,
		...
		if (selectResultList.isEmpty()) {
			ClassifyResultDataDetailEntity classifyResultDataDetailEntity = new ClassifyResultDataDetailEntity();
			...
			for (int classify = 1; classify <= ClassifyScoreAISubLogic.SCORE_CLASSIFICATION_ALL_MAP.size()
					- 1; classify++) {
				List<ClassifyResultDataDetailEntity> detailEntities = new ArrayList<ClassifyResultDataDetailEntity>();
