...
import dev.application.common.commonservice.CopyFileAndGetCsvFile;
...
public class ConditionResultDataLogic {
	...
	/** Logger */
	private static final Logger logger = LoggerFactory.getLogger(ConditionResultDataLogic.class);
	 ...
	 */
	public String execute(boolean updateFlg) {
		...
		String where = "hash = '" + hashData + "'";
		try {
			selectResultList = select.executeSelect(null, UniairConst.BM_M002, selList, where, null, "1");
			if (!selectResultList.isEmpty()) {
				// Entityにマッピングする
				for (List<String> list : selectResultList) {
					ConditionResultDataEntity mapSelectDestination = mappingConditionEntity(0, list);
