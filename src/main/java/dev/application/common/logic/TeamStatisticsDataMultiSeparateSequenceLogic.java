...
/**
 * team_statistics_dataテーブルに登録する前の同一レコードに紐づく通番を分類するロジック
 * @author shiraishitoshio
 *
 */
@Component
public class TeamStatisticsDataMultiSeparateSequenceLogic {

	/** Logger */
	private static final Logger logger = LoggerFactory.getLogger(TeamStatisticsDataMultiSeparateSequenceLogic.class);

	/** クラス名 */
	private static final String CLASS_NAME = TeamStatisticsDataMultiSeparateSequenceLogic.class.getSimpleName();
	...
	public List<String> separateLogic(int seq) {
		...
		logger.info(" TeamStatisticsDataMultiSeparateSequenceLogic start : {} ", CLASS_NAME);
		...
		String team = conditionList.get(0).getHomeTeamName();
		...
		logger.info(" TeamStatisticsDataMultiSeparateSequenceLogic end : {} ", CLASS_NAME);
	...
	private BookDataSelectEntity mappingEntity(List<String> parts) throws Exception {
		...
		mappingDto.setHomeTeamName(parts.get(4));
		return mappingDto;
	}
