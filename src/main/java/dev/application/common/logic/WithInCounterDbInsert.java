...
public class WithInCounterDbInsert {
	...
	public void execute(List<String> dataKeyList,
		...
		List<WithinDataScoredCounterEntity> chkConditionList = new ArrayList<WithinDataScoredCounterEntity>();
		...
		try {
			...
			if (!selectResultList.isEmpty()) {
				...
				for (List<String> list : selectResultList) {
					WithinDataScoredCounterEntity mapSelectDestination = mappingSelectEntity(list);
		...
		if (!chkConditionList.isEmpty()) {
			for (WithinDataScoredCounterEntity entity : chkConditionList) {
		...
		if (!chkFlg) {
			...
			List<WithinDataScoredCounterEntity> conditionList = new ArrayList<WithinDataScoredCounterEntity>();
			try {
				selectResultList = select.executeSelect(null, UniairConst.BM_M017, sel17List,
						where, null, "1");
				if (!selectResultList.isEmpty()) {
			...
			if (!conditionList.isEmpty()) {
				...
				List<WithinDataScoredCounterEntity> insertList = new ArrayList<WithinDataScoredCounterEntity>();
				WithinDataScoredCounterEntity withinDataScoredCounterEntity = new WithinDataScoredCounterEntity();
		...
		selectResultList = null;
		List<WithinDataScoredCounterDetailEntity> chkDetailConditionList = new ArrayList<WithinDataScoredCounterDetailEntity>();
		try {
			...
			if (!selectResultList.isEmpty()) {
				// Entityにマッピングする
				for (List<String> list : selectResultList) {
					WithinDataScoredCounterDetailEntity mapSelectDestination = mappingSelectDetailEntity(list);
		...
		String idDetail = "";
		if (!chkConditionList.isEmpty()) {
			for (WithinDataScoredCounterDetailEntity entity : chkDetailConditionList) {
		...
		if (!chkDetailFlg) {
			...
			selectResultList = null;
			List<WithinDataScoredCounterDetailEntity> condition2List = new ArrayList<WithinDataScoredCounterDetailEntity>();
			try {
				selectResultList = select.executeSelect(null, UniairConst.BM_M018, sel18List,
						where, null, "1");
				if (!selectResultList.isEmpty()) {
			...
			if (!condition2List.isEmpty()) {
				...
				List<WithinDataScoredCounterDetailEntity> insertList = new ArrayList<WithinDataScoredCounterDetailEntity>();
				WithinDataScoredCounterDetailEntity withinDataScoredCounterDetailEntity = new WithinDataScoredCounterDetailEntity();
	 ...
	 * @throws Exception
	 */
	private WithinDataScoredCounterEntity mappingSelectEntity(List<String> parts) throws Exception {
		WithinDataScoredCounterEntity mappingDto = new WithinDataScoredCounterEntity();
	 ...
	 * @throws Exception
	 */
	private WithinDataScoredCounterDetailEntity mappingSelectDetailEntity(List<String> parts) throws Exception {
		WithinDataScoredCounterDetailEntity mappingDto = new WithinDataScoredCounterDetailEntity();
		mappingDto.setId(parts.get(0));
		mappingDto.setCountry(parts.get(1));
		mappingDto.setLeague(parts.get(2));
